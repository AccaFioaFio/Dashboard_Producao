...
export const fatoPedidoItem = sqliteTable(
  'fato_pedido_item',
  {
    id: integer('id').primaryKey({ autoIncrement: true }),
    pedidoNorm: text('pedido_norm').notNull(),
    ...
    tipoComercializacao: text('tipo_comercializacao'),
    status: text('status'),
    valorPedidoTotal: real('valor_pedido_total').notNull().default(0),
    valorPedidoFaturado: real('valor_pedido_faturado').notNull().default(0),
    codProduto: text('cod_produto').notNull(),
    nomeProduto: text('nome_produto'),
    categoriaProduto: text('categoria_produto'),
    pedidoCliente: text('pedido_cliente'),
    qtdPedida: real('qtd_pedida').notNull().default(0),
    qtdFaturada: real('qtd_faturada').notNull().default(0),
    precoBruto: real('preco_bruto'),
    precoLiquido: real('preco_liquido'),
    valorBruto: real('valor_bruto').notNull().default(0),
    valorLiquido: real('valor_liquido').notNull().default(0),
    descontoTotal: real('desconto_total').notNull().default(0),
    dataVenda: text('data_venda'),
  ...
  (table) => [
    index('idx_pedido_item_norm').on(table.pedidoNorm),
    index('idx_pedido_item_cliente').on(table.cliente),
    index('idx_pedido_item_parceiro').on(table.parceiroCodigo),
    index('idx_pedido_item_produto').on(table.codProduto),
    index('idx_pedido_item_venda').on(table.dataVenda),
  ],
)
